        ...
        public void receive(@Payload List<RestaurantApprovalResponseAvroModel> messages,
                ...
                messages.forEach(restaurantApprovalResponseAvroModel -> {
                        try {
                                ...
                                // NO - OP for optimistic locking exception this means the order is already
                                // processed the work and do not throw to prevent reading the data from kafka!
                                log.error("Caught optimistic locking exception in RestaurantApprovalResponseKafkaListener for order id: {}",
                                                restaurantApprovalResponseAvroModel.getOrderId());
                        } catch (OrderNotFoundException e) {
                                // NO-OP for OrderNotFoundException as the order is not found in the system
                                log.error("Order not found for order id: {}",
                                                restaurantApprovalResponseAvroModel.getOrderId());
                        }
                });
