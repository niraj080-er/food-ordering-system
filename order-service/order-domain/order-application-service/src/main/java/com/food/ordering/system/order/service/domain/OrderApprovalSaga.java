...
public class OrderApprovalSaga implements SagaStep<RestaurantApprovalResponse> {
    ...
    public void process(RestaurantApprovalResponse restaurantApprovalResponse) {
        Optional<OrderApprovalOutboxMessage> orderApprovalOutboxMessageRespose = approvalOutboxHelper
                .getApprovalOutboxMessageBySagaIdAndSagaStatus(UUID.fromString(restaurantApprovalResponse.getSagaId()),
                        SagaStatus.PROCESSING);
        if (orderApprovalOutboxMessageRespose.isEmpty()) {
            log.info("An outbox message with saga id: {} is already processed! ",
                    restaurantApprovalResponse.getSagaId());
            return;
        }
        OrderApprovalOutboxMessage orderApprovalOutboxMessage = orderApprovalOutboxMessageRespose.get();
    ...
    public void rollback(RestaurantApprovalResponse restaurantApprovalResponse) {
       ...
       paymentOutboxHelper.savePaymentOutboxMessage(orderDataMapper.orderCancelledEventToorOrderPaymentEventPayload(domainEvent), domainEvent.getOrder().getOrderStatus(), sagaStatus, OutboxStatus.STARTED, UUID.fromString(restaurantApprovalResponse.getSagaId()));


        log.info("Order approval rollback for id {}", domainEvent.getOrder().getId().getValue());
    ...
    private OrderPaymentOutboxMessage getUpdatedPaymentOutboxMessage(String sagaId,
        ...
        if (orderPaymentOutboxMessageResponse.isEmpty()) {
            throw new OrderDomainException(
                    "Payment outox message can not be found in " + SagaStatus.PROCESSING.name() + "states");
        }
